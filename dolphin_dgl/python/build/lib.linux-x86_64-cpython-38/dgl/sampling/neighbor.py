...
def sample_with_ts_recent(indptr,indices,curts,dts,totalts,totaleid,sampleIDs,sampleMask,seedNUM,fanNUM,outSRC,outDST,outts,outeid):
    _indptr = F.to_dgl_nd(indptr)
    _indices = F.to_dgl_nd(indices)
    _curts = F.to_dgl_nd(curts)
    _dts = F.to_dgl_nd(dts)
    _totalts = F.to_dgl_nd(totalts)
    _totaleid = F.to_dgl_nd(totaleid)
    _indices = F.to_dgl_nd(indices)
    _sampleIDs = F.to_dgl_nd(sampleIDs)
    _sampleMask = F.to_dgl_nd(sampleMask)
    _outSRC = F.to_dgl_nd(outSRC)
    _outDST = F.to_dgl_nd(outDST)
    _outts = F.to_dgl_nd(outts)
    _outeid = F.to_dgl_nd(outeid)
    arr = _CAPI_DGLSampleNeighborsWithTsRecent(_indptr,_indices,_curts,_dts,_totalts,_totaleid,_sampleIDs,_sampleMask,seedNUM,fanNUM,_outSRC,_outDST,_outts,_outeid)
    outSRC = utils.toindex(arr(0),dtype='int32').tousertensor()
    outDST = utils.toindex(arr(1),dtype='int32').tousertensor()
    NUMArray = utils.toindex(arr(2),dtype='int32').tousertensor()
    return NUMArray[0]

def sample_with_expired(indptr,indices,curts,totalts,totaleid,sampleIDs,expired,seedNUM,fanNUM,curBlock,zombieBlock,outSRC,outDST,outts,outeid):
    _indptr = F.to_dgl_nd(indptr)
    _indices = F.to_dgl_nd(indices)
    _curts = F.to_dgl_nd(curts)
    _totalts = F.to_dgl_nd(totalts)
    ...
    _indices = F.to_dgl_nd(indices)
    _sampleIDs = F.to_dgl_nd(sampleIDs)
    _expired = F.to_dgl_nd(expired)
    _outSRC = F.to_dgl_nd(outSRC)
    _outDST = F.to_dgl_nd(outDST)
    _outts = F.to_dgl_nd(outts)
    _outeid = F.to_dgl_nd(outeid)
    arr = _CAPI_DGLSampleNeighborsWithExpired(_indptr,_indices,_curts,_totalts,_totaleid,_sampleIDs,_expired,seedNUM,fanNUM,curBlock,zombieBlock,_outSRC,_outDST,_outts,_outeid)
    outSRC = utils.toindex(arr(0),dtype='int32').tousertensor()
    outDST = utils.toindex(arr(1),dtype='int32').tousertensor()
    NUMArray = utils.toindex(arr(2),dtype='int32').tousertensor()
    return NUMArray[0]
