...
int32_t CSRSamplingWithTsRecent(
    IdArray& cached_indptr ,IdArray& cached_indices,IdArray& cached_curts ,IdArray& dts ,IdArray& cached_totalts,IdArray& totaleid,
		IdArray& sampleIDs ,IdArray& sampleMask ,int seedNUM, int fanNUM,
    IdArray& outSRC, IdArray& outDST,IdArray& outts, IdArray& outeid){

		int32_t NUM = impl::CSRSamplingWithTsRecentUniform<kDLGPU, int32_t>(
			cached_indptr,cached_indices,cached_curts,dts,cached_totalts,totaleid,
			sampleIDs,sampleMask,seedNUM,fanNUM,
			outSRC,outDST,outts,outeid);
}

int32_t CSRSamplingWithExpired(
    IdArray& cached_indptr ,IdArray& cached_indices,IdArray& cached_curts ,IdArray& cached_totalts,IdArray& totaleid,
		IdArray& sampleIDs ,IdArray& expired ,int seedNUM, int fanNUM,int cur_block,int zombie_block,
    IdArray& outSRC, IdArray& outDST,IdArray& outts, IdArray& outeid){

		int32_t NUM = impl::CSRSamplingWithExpiredUniform<kDLGPU, int32_t>(
			cached_indptr,cached_indices,cached_curts,cached_totalts,totaleid,
			sampleIDs,expired,seedNUM,fanNUM,cur_block,zombie_block,
			outSRC,outDST,outts,outeid);
}
