...
int32_t SampleWithTsRecent(
  IdArray& indptr ,IdArray& indices,IdArray& curts ,IdArray& dts ,IdArray& totalts,IdArray& totaleid,
  IdArray& sampleIDs ,IdArray& sampleMask ,int seedNUM, int fanNUM,
  IdArray& outSRC, IdArray& outDST, IdArray& outts, IdArray& outeid
  ) {
  int32_t NUM = aten::CSRSamplingWithTsRecent(indptr ,indices , curts,dts, totalts,totaleid, sampleIDs ,sampleMask,seedNUM ,fanNUM ,outSRC ,outDST,outts, outeid); 
  return NUM;
}

int32_t SampleWithExpired(
  IdArray& indptr ,IdArray& indices,IdArray& curts ,IdArray& totalts,IdArray& totaleid,
  IdArray& sampleIDs ,IdArray& expired ,int seedNUM, int fanNUM, int cur_block, int zombie_block,
  IdArray& outSRC, IdArray& outDST, IdArray& outts, IdArray& outeid
  ) {
  int32_t NUM = aten::CSRSamplingWithExpired(indptr ,indices , curts, totalts,totaleid, sampleIDs ,expired,seedNUM ,fanNUM , cur_block, zombie_block, outSRC ,outDST,outts, outeid); 
  return NUM;
}
...
DGL_REGISTER_GLOBAL("sampling.neighbor._CAPI_DGLSampleNeighborsWithTsRecent")
.set_body([] (DGLArgs args, DGLRetValue *rv) {
      IdArray cached_indptr = args[0];
      IdArray cached_indices = args[1];
      IdArray cached_curts = args[2];
      IdArray dts = args[3];
      IdArray cached_totalts = args[4];
      IdArray totaleid = args[5];
      IdArray sampleIDs = args[6];
      IdArray sampleMask = args[7];
      int seedNUM = args[8];
      int fanNUM = args[9];
      IdArray outSRC = args[10];
      IdArray outDST = args[11];
      IdArray outts = args[12];
      IdArray outeid = args[13];
      int NUM = sampling::SampleWithTsRecent(
        cached_indptr,cached_indices,cached_curts,dts,cached_totalts,totaleid,sampleIDs,sampleMask,seedNUM,fanNUM,outSRC,outDST,outts,outeid);
      IdArray NUMArray = Full(NUM ,1 , sizeof(int)*8,cached_indptr->ctx);
      *rv = ConvertNDArrayVectorToPackedFunc({outSRC, outDST, NUMArray});
  });


DGL_REGISTER_GLOBAL("sampling.neighbor._CAPI_DGLSampleNeighborsWithExpired")
.set_body([] (DGLArgs args, DGLRetValue *rv) {
      IdArray cached_indptr = args[0];
      IdArray cached_indices = args[1];
      IdArray cached_curts = args[2];
      IdArray cached_totalts = args[3];
      IdArray totaleid = args[4];
      IdArray sampleIDs = args[5];
      IdArray expired = args[6];
      int seedNUM = args[7];
      int fanNUM = args[8];
      int curBlock = args[9];
      int zombieBlock = args[10];
      IdArray outSRC = args[11];
      IdArray outDST = args[12];
      IdArray outts = args[13];
      IdArray outeid = args[14];
      int NUM = sampling::SampleWithExpired(
        cached_indptr,cached_indices,cached_curts,cached_totalts,totaleid,sampleIDs,expired,seedNUM,fanNUM,curBlock,zombieBlock,outSRC,outDST,outts,outeid);
      IdArray NUMArray = Full(NUM ,1 , sizeof(int)*8,cached_indptr->ctx);
      *rv = ConvertNDArrayVectorToPackedFunc({outSRC, outDST, NUMArray});
  });
