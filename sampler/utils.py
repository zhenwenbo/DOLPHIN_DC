...
def print_total():
    print(f"total save {total_save}GB")
...
def print_IO():
    print(f"read IO: {read_IO:.2f}GB use time: {read_IO_time:.2f}s write IO:{write_IO:.2f}GB write IO use time: {write_IO_time:.2f}s")
...
def loadBinDisk(path, ind, use_slice = False):
    ...
    print(f"read disk use time {time_use:.4f}s shape:{res.shape} {res.numel() * 4 / 1024 ** 2 / time_use:.2f}MB/s path: {path}")
    return res
...
def updateBinDisk(path, values, ind, use_slice = False):
    ...
    print(f"write disk node memory use time {time.time() - update_disk_s:.4f}s shape:{values.shape}")
