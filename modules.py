...
class GeneralModel(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, sample_param, memory_param, gnn_param, train_param, emb_buffer = None, combined=False):
        ...
        if memory_param['type'] == 'node':
            ...
            elif memory_param['memory_update'] == 'gru':
        ...
        for l in range(self.gnn_param['layer']):
            ...
            for h in range(self.sample_param['history']):
                ...
                if l != self.gnn_param['layer'] - 1:
                    if (self.emb_buffer and l == self.gnn_param['layer'] - 2 and self.emb_buffer.cur_mode == 'train' and self.emb_buffer.use_buffer):
                        rst = self.emb_buffer.input_his_emb(rst)
                        self.emb_buffer.update_his_emb(rst)
