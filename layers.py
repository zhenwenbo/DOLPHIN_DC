...
class HotEncode(torch.nn.Module):

    def __init__(self, dim):
        super(HotEncode, self).__init__()
        self.dim = dim
        self.w = torch.nn.Linear(1, dim)
    

    def forward(self, t):
        output = self.w(t.reshape((-1, 1)))
        return output
...
class SAGE(torch.nn.Module):
    def __init__(self, in_size, hid_size, out_size,num_layers=2,dropout=0.5):
        ...
        self.layer.append(dglnn.SAGEConv(in_size, hid_size, 'mean'))
...
class DTMPLayer(torch.nn.Module):
    ...
    def __init__(self, dim_node_feat, dim_edge_feat, dim_time, num_head, dropout, att_dropout, dim_out, seperate = False):
        ...
        torch.nn.init.normal_(self.att_bias, mean = 0, std = 0.01)
        if dim_time > 0:
            self.time_enc = TimeEncode(dim_time)
        if seperate:
            if dim_edge_feat > 0:
