...
saveBin(start, f'/DOLPHIN/data/{data}/simple_start-{batch_size}.bin')
saveBin(end, f'/DOLPHIN/data/{data}/simple_end-{batch_size}.bin')
saveBin(IDs, f'/DOLPHIN/data/{data}/simple_IDs-{batch_size}.bin')
...
def gen_simple(start, end, IDs):
    ...
    saveBin(start_cache_ind, f'/DOLPHIN/data/{data}/start_cache_ind-{batch_size}.bin')
    saveBin(start_flush_ind, f'/DOLPHIN/data/{data}/start_flush_ind-{batch_size}.bin')
    saveBin(end_flush_idx, f'/DOLPHIN/data/{data}/end_flush_idx-{batch_size}.bin')
    saveBin(start_cache_ptr, f'/DOLPHIN/data/{data}/start_cache_ptr-{batch_size}.bin')
    saveBin(start_flush_ptr, f'/DOLPHIN/data/{data}/start_flush_ptr-{batch_size}.bin')
    saveBin(end_flush_ptr, f'/DOLPHIN/data/{data}/end_flush_ptr-{batch_size}.bin')
...
saveBin(torch.tensor([max_num], dtype = torch.int32), f'/DOLPHIN/data/{data}/simple_max_num-{batch_size}.bin')
...
def uni_simple(node, block, freq, reorder_res):
    ...
    dis_node_idx = torch.nonzero(count_labels == freq).reshape(-1)
    res = None
    ...
    res_uni_inv_sort, res_uii = torch.sort(res_uni_inv) 
    ...
    res_uni_inv_threshold_mask = ~torch.isin(res_uni_inv_sort, res_uni_inv_threshold_ind) 
    
    res_ret = dis_node_idx[res_uii[res_uni_inv_threshold_mask]] 
...
if root_dir not in sys.path:
    sys.path.append(root_dir)
from utils import *
saveBin(res.cpu(), f'/DOLPHIN/data/{data}/node_simple_reorder_map-{batch_size}.bin')

path = f'/DOLPHIN/data/{data}'
...
print(f"node_memory total use time: {time.time() - total_start:.2f}s")